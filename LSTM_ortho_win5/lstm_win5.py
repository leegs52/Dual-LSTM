...
class MLSTM(nn.Module):
    ...
    def __init__(self, input_dim=100, hidden_dim=3, sequence_length=7, output_dim=1, batch_size=2):
        ...
        self.hidden_cell = (torch.zeros(1, self.batch_size, self.hidden_dim).to(self.device),  # .to(self.device),when you use gpu
                            torch.zeros(1, self.batch_size, self.hidden_dim).to(self.device))  # .to(self.device)) when you use gpu
    ...
    def init_hidden(self):
        ...
        self.hidden_cell = (torch.rand(1, self.batch_size, self.hidden_dim).to(self.device),  # .to(self.device),when you use gpu
...
if __name__=='__main__':
    ...
    input_dim = 5
    #hidden_dim = 3
    seq_len = 5 # we suppose last 7 days data affect to CO2 level of next day
    output_dim = 1
    batch_size=3
    epochs = 50
    ...
    with open('./result_lstm_win5.csv','w',encoding='utf_8_sig') as f:
        f.write('window,hidden,lr,lambda1,lambda2,lambda3,val_MSE,train_MSE,epoch\n')
        
    for hidden_dim in hidden_dims:
        for lr in lrs:
            model = MLSTM(input_dim=input_dim, 
            ...
            for i in range(epochs):
                ...
                with torch.no_grad():
                    ...
                    if i==0:
                        ...
                        min_mse_train=float(total_loss.item()/len(train_loader))
            ...
            print(best_params)
            with open('./result_lstm_win5.csv','a',encoding='utf_8_sig') as f:
